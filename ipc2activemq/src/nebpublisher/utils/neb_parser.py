...
class Parser():
  ...
  def parse_service_check(self, original_message):
    if original_message == None:
      return BAD_FORMAT

    logger.debug("Message %s - service check" % original_message)
    ...
    if not notification:
      logger.debug('Notification is DISABLED for this service check! Ignoring...')
      return NOT_IMPLEMENTED
    host = data[1]
    command_name = data[2]
    state = data[3]
